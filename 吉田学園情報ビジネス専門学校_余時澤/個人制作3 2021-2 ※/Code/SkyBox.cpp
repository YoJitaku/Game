...
void DrawSkyBox(void)
{
	//g_nCntTime++;
	LPDIRECT3DDEVICE9 pDevice = GetDevice();
	D3DXMATRIX mtxTrans, mtxRot, matRotSky, WorldMtxSkyBox;
	for (int nCnt = 0; nCnt < SKYBOX_POLY; nCnt++)
	{
		D3DXMatrixIdentity(&WorldMtxSkyBox); //単位行列
		...
		D3DXMatrixMultiply(&WorldMtxSkyBox, &WorldMtxSkyBox, &mtxRot); //世界matrixの周り　Param1 out put matrix = 世界matrix * 回されたmatrix
		D3DXMatrixTranslation(&mtxTrans, pSkyBox[nCnt].pos.x, pSkyBox[nCnt].pos.y, pSkyBox[nCnt].pos.z); //世界matrix全体　移動したい距離というmatrixの合算
		D3DXMatrixMultiply(&WorldMtxSkyBox, &WorldMtxSkyBox, &mtxTrans); //世界matrixの移動
		pDevice->SetTransform(D3DTS_WORLD, &WorldMtxSkyBox); //set the world into Map
