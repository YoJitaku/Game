...
RANKPLAYER g_aRankPlayer[6];//fileから読込んだdata, score and name
int g_nCntAnimeRk = 0;//時間count
char g_Input;//入力した文字を保存
bool bInput;//入力判断
NAME g_aName[RESULT_ROW][6];//名前polygonのmatrix
...
void InputNewScore(RANKPLAYER *pRankPlayer)
{
	int nTop, nHistory;
	char aTop[256];
	for (int nCountS = 0; nCountS < 5; nCountS++)pRankPlayer[nCountS].nHistory = 0;//history初期化
	pRankPlayer[5].nScore = GetScore();//player's score
	pRankPlayer[5].nHistory = 1;//new input mark

	//compositor //z = x, x = y, y = z
	for (int nCount2 = 0; nCount2 < 5; nCount2++)
	{
		nHistory = pRankPlayer[nCount2].nHistory;//new!! mark
...
void SetRank(void)
{
	...
	g_aRank[1].pos = D3DXVECTOR3(1300, 225, 0.0f);
	g_aRank[1].size = D3DXVECTOR2(100, 50);
	//score
	for (int nCntResultRow = 0; nCntResultRow < RESULT_ROW; nCntResultRow++)
	{
		for (int nCntResultZero = 0; nCntResultZero < MAX_COMPONENT_RESULT; nCntResultZero++)
		{
			...
			g_aResult[nCntResultRow][nCntResultZero].pos.x = 400 + (float)nCntResultZero * 50;
			g_aResult[nCntResultRow][nCntResultZero].pos.y = 225 + (float)nCntResultRow * 100;
			g_aResult[nCntResultRow][nCntResultZero].pos.z = 0;
			g_aResult[nCntResultRow][nCntResultZero].size.x = 25;
			g_aResult[nCntResultRow][nCntResultZero].size.y = 35;
	...
	for (int nCntrow = 0; nCntrow < RESULT_ROW; nCntrow++)
	{
		for (int nCnt = 0; nCnt < 6; nCnt++)
		{
			...
			g_aName[nCntrow][nCnt].pos.x = 900 + (float)nCnt * 50;
			g_aName[nCntrow][nCnt].pos.y = 225 + (float)nCntrow * 100;
			g_aName[nCntrow][nCnt].size.x = 25;
			g_aName[nCntrow][nCnt].size.y = 30;
...
HRESULT InitRank(void)
{
	...
	D3DXCreateTextureFromFile(pDeviceRk, "data/TEXTURE/RankScore.png", &g_pTextureRank[2]);
...
{
	VERTEX_2D *pVtx;
	int nnscore = GetScore();
	RANKPLAYER *pRankPlayer = &g_aRankPlayer[0];

	if ((GetkeyboardTrigger(DIK_RETURN) == true && bInput == true))
		SetFade(FADE_OUT, MODE_TITLE2);

	if (GetkeyboardTrigger(DIK_RETURN) == true && (nnscore < pRankPlayer->nScore))
		SetFade(FADE_OUT, MODE_TITLE2);

	g_nCntAnimeRk++;

	int nSigma = 0;//頂点番号を記録pvtx
	//score
	for (int nCntRow = 0; nCntRow < RESULT_ROW; nCntRow++, pRankPlayer++)
	{
		...
		{
			g_aRank[1].bUse = true;
			g_aRank[1].pos.y = 225 + (float)(nCntRow * 100);
			g_pVtxBuffRank->Lock(0, 0, (void**)&pVtx, 0);
	...
	for (int nCntrow = 0; nCntrow < RESULT_ROW; nCntrow++, pRankPlayer++)
	{
		for (int nCnt = 0; nCnt < 6; nCnt++, pVtx += 4)
		{
			...
			{
				g_aName[nCntrow][nCnt].nType = 3;//文字以外の描画しません
				//cursorを点滅する
			...
			else
			{
				...
				else g_aName[nCntrow][nCnt].bUse = false;
			}
...
void DrawRank(void)
{
	...
	pDeviceRk->SetStreamSource(0, g_pVtxBuffRank, 0, sizeof(VERTEX_2D));//explosion画像の頂点Data flowの設定
	pDeviceRk->SetFVF(FVF_VERTEX_2D);//頂点modの設定
	...
	for (int nCntRk = 0; nCntRk < MAX_COMPONENT_RANK; nCntRk++, pRank++)
	{
		if (pRank->bUse == true)
		{
			pDeviceRk->SetTexture(0, g_pTextureRank[nCntRk]);//explosion画像設定
