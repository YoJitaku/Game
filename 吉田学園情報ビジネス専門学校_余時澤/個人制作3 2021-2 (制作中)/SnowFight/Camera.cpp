...
HRESULT InitCamera(void)
{
	...
	pCamera[0].posR.x = pCamera[0].posV.x + 2.f * SightVector.x;
	pCamera[0].posR.z = pCamera[0].posV.z + 2.f * SightVector.z;
	pCamera[0].posR.y = 20.f;
...
void SetCamera(void)
{
	...
	D3DXMatrixIdentity(&pCamera[nCnt].mtxProjection); //cameraの単位行列
	//今のmatrixはprojection matrixにする. Param2　y軸上の視角size -0.1f->1.0f, Param3 高さ/横さ, 
	D3DXMatrixPerspectiveFovLH(&pCamera[nCnt].mtxProjection, D3DXToRadian(45.0f), 1920.0f / 1080.0f, 10.0f, 1000000000000.0f);
	pDevice->SetTransform(D3DTS_PROJECTION, &pCamera[nCnt].mtxProjection);
	D3DXMatrixIdentity(&pCamera[nCnt].mtxView);
	D3DXMatrixLookAtLH(&pCamera[nCnt].mtxView, &pCamera[nCnt].posV, &pCamera[nCnt].posR, &pCamera[nCnt].vecV);
	pDevice->SetTransform(D3DTS_VIEW, &pCamera[nCnt].mtxView);
}
