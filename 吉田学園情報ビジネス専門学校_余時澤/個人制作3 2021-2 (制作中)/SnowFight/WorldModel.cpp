...
void InitWorldModel(void)
{
	...
	for (int nCnt = 0; nCnt < WORLDMODEL_NUM; nCnt++)
	{
		...
		SetCollisionBox(nNumVtx, sizeFvF, pVtxBuff);
		...
		//	//当たり判定用boxの頂点を探す
		//	if (MaxCollisionBox[nCnt] == MinCollisionBox[nCnt])//初めの頂点
		//		MaxCollisionBox[nCnt] = vtx;
		//	else
		//	{
		//		if (vtx.x > MaxCollisionBox[nCnt].x) MaxCollisionBox[nCnt].x = vtx.x;
		//		else if (vtx.x < MinCollisionBox[nCnt].x) MinCollisionBox[nCnt].x = vtx.x;
		//		if (vtx.y > MaxCollisionBox[nCnt].y) MaxCollisionBox[nCnt].y = vtx.y;
		//		else if (vtx.x < MinCollisionBox[nCnt].x) MinCollisionBox[nCnt].y = vtx.y;
		//		if (vtx.z > MaxCollisionBox[nCnt].z) MaxCollisionBox[nCnt].z = vtx.z;
		//		else if (vtx.x < MinCollisionBox[nCnt].x) MinCollisionBox[nCnt].z = vtx.z;
		//	}
		//}
